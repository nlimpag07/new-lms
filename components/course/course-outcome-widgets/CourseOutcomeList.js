...
const CourseOutcomeList = (props) => {
  ...
  if (outcomeList) {
    outcomeList.map((dataItem) => {
      ...
      let theoutcome = {
        ...
        visibilityNumber:dataItem.visibility
      };
      dataList.push(theoutcome);
    });
  ...
  const removeSelected = (item) => {
    ...
    setOutcomeList(newDataList); */
    var config = {
      method: "delete",
      url: apiBaseUrl + "/CourseOutcome/" + item.id,
      headers: {
  ...
  return (
    ...
    <Row className="widget-container">
      ...
      {/* <CourseCircularUi /> */}
      <style jsx global>{`
        .CourseOutcomeList h1 {
          font-size: 2rem;
          font-weight: 700;
        }
        .CourseOutcomeList .k-grid-header {
