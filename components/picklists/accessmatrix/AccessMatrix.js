...
const AccessMatrix = ({ data }) => {
  //console.log("data", data);
  const router = useRouter();
  var [modal2Visible, setModal2Visible] = useState((modal2Visible = false));
  var [rolesModal, setAccessMatrixModal] = useState({
    visible: false,
    modalOperation: "",
    dataProps: null,
    width: "auto",
  ...
  const [allAccessMatrixData, setAllAccessMatrixData] = useState([]);
  const [rolesData, setAccessMatrixData] = useState([]);
        ...
        try {
          ...
          if (response) {
            ...
            if (theRes) {
              ...
              setAllAccessMatrixData(result);
              setAccessMatrixData(result);
              setSpin(false);
            } else {
            }
          }
  ...
  function onSearch(val) {
    ...
    let searchedData = allAccessMatrixData.filter((d) =>
      d.name.toLowerCase().includes(val.toLowerCase())
    );
    
    setAccessMatrixData(searchedData);
  }
  useEffect(() => {
    if (searchLoading) {
      setSpin(false);
    ...
    <motion.div initial="hidden" animate="visible" variants={list}>
      ...
      >
        <div className="common-holder">
          <Col
            className="gutter-row widget-holder-col AccessMatrix"
            xs={24}
            sm={24}
            md={24}
            lg={24}
          >
            <h1>Picklists: AccessMatrix</h1>
            <Row justify="start">
            ...
            <Row className="PicklistAccessMatrix">
      ...
      <Modal
        title={`AccessMatrix - ${rolesModal.modalOperation}`}
        centered
        ...
        className="PicklistAccessMatrixModal"
      >
      ...
      <style jsx global>{`
        ...
        .PicklistAccessMatrixModal .ant-modal-footer {
          display: none;
          opacity: 0;
        }
      `}</style>
