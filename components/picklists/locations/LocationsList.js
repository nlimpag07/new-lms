...
const LocationsList = ({
  ...
  const the_data = locationsData && locationsData.length ? locationsData : [];
  ...
  return (
    <>
      ...
      >
        ...
        <Column field="code" title="Location Code" />
...
function deleteConfirm(e, data, setRunSpin) {
  ...
  var config = {
    method: "delete",
    url: apiBaseUrl + "/picklist/location/" + data.id,
    headers: {
      ...
      if (response) {
        ...
        if (theRes) {
          Modal.success({
            ...
            content: "You have successfully deleted a Location!",
            centered: true,
            width: 450,
            onOk: () => {
              visible: false;
