...
import LearnersCalendar from "../../../components/learners-calendar/LearnersCalendar";
  ...
  return (
    <MainThemeLayout>
      <Layout className="main-content-holder courses-class" id="courses-class">
        <LearnersCalendar listOfTranscripts={transcripts} />
      </Layout>
      <style jsx global>{`
        /* .status-col {
          background: #eeeeee;
...
Transcript.getInitialProps = async (ctx) => {
  ...
  var config = {
    method: "get",
    url: apiBaseUrl + "/learner/MyTranscript",
    headers: {
  ...
  return { transcripts: data, token: token, apiBaseUrl: apiBaseUrl };
};
