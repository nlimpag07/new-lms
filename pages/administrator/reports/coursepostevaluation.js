...
import CoursePostEvaluations from "../../../components/reports/course-post-evaluation/CoursePostEvaluations";
...
const PicklistCoursePostEvaluationsIndex = ({ data, token, apiBaseUrl }) => {
  ...
  return (
    <MainThemeLayout>
      <Layout className="main-content-holder courses-class" id="courses-class">
        <CoursePostEvaluations data={data} />
      </Layout>
      <style jsx global>{`
        /* .status-col {
          background: #eeeeee;
...
PicklistCoursePostEvaluationsIndex.getInitialProps = async (ctx) => {
...
export default withAuth(PicklistCoursePostEvaluationsIndex);
